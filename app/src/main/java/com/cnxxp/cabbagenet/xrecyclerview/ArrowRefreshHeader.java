...
public class ArrowRefreshHeader extends LinearLayout implements BaseRefreshHeader{
	...
	public void setState(int state) {
		...
		if (state == BaseRefreshHeader.STATE_REFRESHING) {	// 显示进度
		...
		switch(state){
            case BaseRefreshHeader.STATE_NORMAL:
                if (mState == BaseRefreshHeader.STATE_RELEASE_TO_REFRESH) {
                    mArrowImageView.startAnimation(mRotateDownAnim);
                }
                if (mState == BaseRefreshHeader.STATE_REFRESHING) {
                    mArrowImageView.clearAnimation();
                }
	...
	public void refreshComplate(){
        ...
        setState(BaseRefreshHeader.STATE_DONE);
    ...
    public void onMove(float delta) {
        if(getVisiableHeight() > 0 || delta > 0) {
            ...
            if (mState <= BaseRefreshHeader.STATE_RELEASE_TO_REFRESH) { // 未处于刷新状态，更新箭头
                if (getVisiableHeight() > mMeasuredHeight) {
                    setState(BaseRefreshHeader.STATE_RELEASE_TO_REFRESH);
                }else {
    ...
    public boolean releaseAction() {
        ...
        if(getVisiableHeight() > mMeasuredHeight &&  mState < BaseRefreshHeader.STATE_REFRESHING){
            setState(BaseRefreshHeader.STATE_REFRESHING);
            isOnRefresh = true;
        }
        // refreshing and header isn't shown fully. do nothing.
        if (mState == BaseRefreshHeader.STATE_REFRESHING && height <=  mMeasuredHeight) {
            //return;
        }
